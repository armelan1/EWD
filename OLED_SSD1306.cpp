...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
OLED_SSD1306::OLED_SSD1306(uint8_t w, uint8_t h, SPIClass *spi,
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
OLED_SSD1306::OLED_SSD1306(int8_t mosi_pin, int8_t sclk_pin,
...
/*!
    @brief  DEPRECATED constructor for SPI SSD1306 displays, using native
            ...
            with the current library. Screen size is determined by enabling
            one of the SSD1306_* size defines in OLED_SSD1306.h. New
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
OLED_SSD1306::OLED_SSD1306(int8_t dc_pin, int8_t rst_pin,
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
OLED_SSD1306::OLED_SSD1306(int8_t rst_pin) :
...
inline void OLED_SSD1306::SPIwrite(uint8_t d) {
  if(spi) {
    (void)spi->transfer(d);
  } else {
...
void OLED_SSD1306::ssd1306_command1(uint8_t c) {
  if(wire) { // I2C
...
void OLED_SSD1306::ssd1306_commandList(const uint8_t *c, uint8_t n) {
  if(wire) { // I2C
    wire->beginTransmission(i2caddr);
    WIRE_WRITE((uint8_t)0x00); // Co = 0, D/C = 0
    uint8_t bytesOut = 1;
...
*/
boolean OLED_SSD1306::begin(uint8_t vcs, uint8_t addr, boolean reset,
  boolean periphBegin) {
...
//  } else {
      ...
      splash2_data, splash2_width, splash2_height, SSD1306_WHITE);
//    drawBitmap((WIDTH - splash2_width) / 2, (HEIGHT - splash2_height) / 2,
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void OLED_SSD1306::drawFastHLine(
...
void OLED_SSD1306::drawFastHLineInternal(
  int16_t x, int16_t y, int16_t w, uint16_t color) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void OLED_SSD1306::drawFastVLine(
...
void OLED_SSD1306::drawFastVLineInternal(
...
*/
boolean OLED_SSD1306::getPixel(int16_t x, int16_t y) {
  if((x >= 0) && (x < width()) && (y >= 0) && (y < height())) {
    // Pixel is in-bounds. Rotate coordinates if needed.
    switch(getRotation()) {
     case 1:
...
*/
uint8_t *OLED_SSD1306::getBuffer(void) {
  return buffer;
}
...
*/
void OLED_SSD1306::invertDisplay(boolean i) {
  TRANSACTION_START
